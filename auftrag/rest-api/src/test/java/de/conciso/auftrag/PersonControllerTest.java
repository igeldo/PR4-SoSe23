...
class PersonControllerTest {
  ...
  class Given_Person_can_be_created {
    ...
    @BeforeEach
    void arrange() {
      given(personen.create(anyString(), anyString())).willReturn(new Person("Hugo", "Hugo"));
    }
    ...
    class When_calling_create {
      ...
      @BeforeEach
      void act() {
        result = cut.create("Hugo");
      }

      @Test
      void then_PersonenService_is_called() {
        verify(personen).create("Hugo", "Hugo");
      }
      ...
      void then_body_is_correct() {
        var expected = new PersonRepresentation("Hugo", "Hugo");
